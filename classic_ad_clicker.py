...
def main():
    ...
    search_classic_controller = None

    try:
        search_classic_controller = SearchClassicController(driver, query, country_code)

        if args.id:
            search_classic_controller.set_browser_id(args.id)

        if args.device_id:
            search_classic_controller.assign_android_device(args.device_id)

        ads, non_ad_links, shopping_ads = search_classic_controller.search_for_ads(non_ad_domains=domains)
        print("ads alink")
        print(ads)
        print(non_ad_links)
        print(shopping_ads)
        ...
        if not (ads or shopping_ads or non_ad_links):
            ...
            if config.behavior.telegram_enabled:
                notify_matching_ads(query, links=None, stats=search_classic_controller.stats)
        else:
            ...
            search_classic_controller.click_shopping_ads(shopping_ads)
            search_classic_controller.click_links(all_links)

            if config.behavior.hooks_enabled:
                hooks.after_clicks_hook(driver)

            if config.behavior.telegram_enabled:
                notify_matching_ads(query, links=ads + shopping_ads, stats=search_classic_controller.stats)

            logger.info(search_classic_controller.stats)
        ...
        if search_classic_controller:
            if config.behavior.hooks_enabled:
                hooks.before_browser_close_hook(driver)

            search_classic_controller.end_search()
