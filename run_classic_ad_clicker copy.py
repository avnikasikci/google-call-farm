...
from live_logger import logger, update_log_formats
...
def main():
    ...
    if args.proxy:
        ...
        live_logger.debug(f"Proxies: {proxies}")
    ...
    try:
        ...
        min = config.behavior.min_non_ads_click
        max = config.behavior.min_non_ads_click
        randClickCount = random.randint(min, max)
        logger.debug(f"*******İşlemler başlatıldı, toplam {randClickCount} adet reklamsız tıklama araması yapılacak.")
        query_index = 0  # Başlangıçta 0'dan başla
    ...
    try:
        ...
        query_index = 0  # Başlangıçta 0'dan başla
        for query in queries:
            # Eğer reklamsız tıklama işlemleri tamamlandıysa, normal sorguları işle
            if query_index >= randClickCount:
                pass
            query_index += 1
