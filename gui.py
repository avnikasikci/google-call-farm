...
class PathsFrame(customtkinter.CTkFrame):
    ...
    def __init__(self, master) -> None:
        ...
        self._page_in_query_file = self._add_path_input(
            row=2, label="Page in Query file", default_value=config.paths.page_in_query_file
        )
        self._proxy_file = self._add_path_input(
            row=3, label="Proxy file", default_value=config.paths.proxy_file
        )
        self._user_agents = self._add_path_input(
            row=4, label="User agents", default_value=config.paths.user_agents
        )
        self._filtered_domains = self._add_path_input(
            row=5, label="Filtered domains", default_value=config.paths.filtered_domains
        )
    ...
    def get_paths(self) -> dict[str, str]:
        ...
        return {
            ...
            "page_in_query_file": self._page_in_query_file.get("1.0", "end-1c"),
...
class BehaviorFrame(customtkinter.CTkFrame):
    ...
    def __init__(self, master) -> None:
        ...
        self._min_non_ads_click = self._add_input_field(
            row=1,
            column=0,
            label="Non Ads Click Min",
            default_value=config.behavior.min_non_ads_click,
        )
        self._max_non_ads_click = self._add_input_field(
            row=1,
            column=3,
            label="Non Ads Click Max",
            default_value=config.behavior.max_non_ads_click,
        )

        self._query_input = self._add_input_field(
            row=2, column=0, label="Query", default_value=config.behavior.query
        )
        ...
        self._page_min_wait_input = self._add_input_field(
            row=3,
            column=0,
            label="Ad page min wait",
            default_value=config.behavior.page_min_wait,
        )
        self._page_max_wait_input = self._add_input_field(
            row=3,
            column=3,
            label="Ad page max wait",
            default_value=config.behavior.page_max_wait,
        )
        ...
        self._check_only_adsclick_domain_value = self._add_checkbox(
            row=9, column=0, label="Only Ads Click Filtered Domain", enabled=config.behavior.check_only_adsclick_domain
        )
        self._random_mouse_value = self._add_checkbox(
            row=9, column=2, label="Random mouse", enabled=config.behavior.random_mouse
        )
        self._custom_cookies_value = self._add_checkbox(
            row=9, column=3, label="Custom cookies", enabled=config.behavior.custom_cookies
        )
        self._hooks_enabled_value = self._add_checkbox(
            row=9, column=4, label="Hooks enabled", enabled=config.behavior.hooks_enabled
        )
        self._telegram_enabled_value = self._add_checkbox(
            row=9, column=5, label="Telegram enabled", enabled=config.behavior.telegram_enabled
        )
        self._send_to_android_value = self._add_checkbox(
            row=9, column=6, label="Send to Android", enabled=config.behavior.send_to_android
        )
    ...
    def get_behavior_config(self) -> dict[str, str]:
        ...
        return {
            ...
            "min_non_ads_click": int(self._min_non_ads_click.get("1.0", "end-1c")),
            "max_non_ads_click": int(self._max_non_ads_click.get("1.0", "end-1c")),
            "page_min_wait": int(self._page_min_wait_input.get("1.0", "end-1c")),
            "page_max_wait": int(self._page_max_wait_input.get("1.0", "end-1c")),
            "max_scroll_limit": int(self._max_scroll_limit_input.get("1.0", "end-1c")),
            "check_only_adsclick_domain": self._check_only_adsclick_domain_value.get(),
...
class ActionButtonsFrame(customtkinter.CTkFrame):
    ...
    def __init__(self, master) -> None:
        ...
        self.run_button_4 = customtkinter.CTkButton(
            self,
            text="Run run_classic_ad_clicker_script.py",
            height=self.relative_height * 3,
            command=master.run_classic_ad_clicker_script,
        )
        self.run_button_4.grid(row=6, column=0, columnspan=6, padx=10, pady=5, sticky="ew")
...
class ConfigGUI(customtkinter.CTk):
    ...
    def run_classic_ad_clicker_script(self):
